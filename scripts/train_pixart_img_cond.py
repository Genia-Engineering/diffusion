#!/usr/bin/env python3
"""PixArt-Sigma 图像条件微调训练入口脚本。

用法:
  # 多卡训练
  accelerate launch scripts/train_pixart_img_cond.py --config configs/pixart_sigma_img_cond_floorplan.yaml

  # 恢复训练
  accelerate launch scripts/train_pixart_img_cond.py \
    --config configs/pixart_sigma_img_cond_floorplan.yaml --resume latest

  # 使用 VAE 编码器
  accelerate launch scripts/train_pixart_img_cond.py \
    --config configs/pixart_sigma_img_cond_floorplan.yaml \
    --override model.image_encoder.type=vae
"""
...
from trainers.pixart_img_cond_trainer import PixArtImgCondTrainer


def parse_args():
    parser = argparse.ArgumentParser(description="Train PixArt-Sigma with image conditioning")
...
def main():
    ...
    trainer = PixArtImgCondTrainer(config)
    trainer.train()
