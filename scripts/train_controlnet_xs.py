#!/usr/bin/env python3
"""ControlNet-XS 训练入口脚本。

用法:
  # 单卡训练
  python scripts/train_controlnet_xs.py --config configs/controlnet_xs_sdxl.yaml

  # 多卡训练 (accelerate)
  accelerate launch scripts/train_controlnet_xs.py --config configs/controlnet_xs_sdxl.yaml

  # 恢复训练
  accelerate launch scripts/train_controlnet_xs.py \
    --config configs/controlnet_xs_sdxl.yaml \
    --resume latest
"""
...
from trainers.controlnet_xs_trainer import ControlNetXSTrainer


def parse_args():
    parser = argparse.ArgumentParser(description="Train ControlNet-XS for Stable Diffusion")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Path to training config YAML (e.g. configs/controlnet_xs_sdxl.yaml)",
    )
    parser.add_argument(
        "--base_config",
        type=str,
...
def main():
    ...
    trainer = ControlNetXSTrainer(config)
    trainer.train()
