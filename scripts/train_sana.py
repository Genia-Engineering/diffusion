#!/usr/bin/env python3
"""Sana 0.6B 全参数微调训练入口脚本。

用法:
  # 单卡训练
  python scripts/train_sana.py --config configs/sana_floorplan.yaml

  # 多卡训练 (accelerate)
  accelerate launch scripts/train_sana.py --config configs/sana_floorplan.yaml

  # 恢复训练
  accelerate launch scripts/train_sana.py \
    --config configs/sana_floorplan.yaml --resume latest
"""
...
from trainers.sana_trainer import SanaTrainer


def parse_args():
    parser = argparse.ArgumentParser(description="Train Sana 0.6B (full fine-tuning)")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
