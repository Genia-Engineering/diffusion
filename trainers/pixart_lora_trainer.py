"""PixArt-Sigma LoRA 微调训练器 — 支持 Flow Matching 和 DDPM 双噪声范式 + 低秩适配。
...
权重冻结策略:
  冻结 → VAE, T5 Text Encoder, PixArtTransformer2DModel 基础权重
  可训练 → LoRA 参数 (lora_A, lora_B)
"""
...
class PixArtLoRATrainer(BaseTrainer):
    """PixArt-Sigma LoRA 微调训练器。"""
    ...
    def _inject_lora(self):
        ...
        logger.info(f"LoRA injected into PixArt Transformer: {len(injected)} layers")
    ...
    def train(self):
        ...
        # ── 阶段2：T5 文本嵌入预计算（须在 accelerator.prepare 之前完成以避免 OOM）──
        # T5-XXL fp32 ~18GB，A10G 22GB 显存紧张。
        ...
        optimizer = self.setup_optimizer(trainable_params=lora_params)
        ...
        if resume_dir:
            ...
            for _ in range(self.global_step):
                lr_scheduler.step()
            logger.info(f"Resumed from step {self.global_step}, lr_scheduler re-synced")
            ...
            for batch in dataloader:
                ...
                with self.accelerator.accumulate(self.transformer):
                    ...
                    if self.accelerator.sync_gradients:
                        grad_norm = self.accelerator.clip_grad_norm_(lora_params, max_grad_norm)
    ...
    def _training_step(self, batch) -> torch.Tensor:
        ...
        if self.noise_paradigm == "flow_matching":
            ...
            return self._training_step_ddpm(latents, noise, bsz, prompt_embeds, attention_mask, weight_mask)

    def _training_step_fm(self, latents, noise, bsz, prompt_embeds, attention_mask) -> torch.Tensor:
    ...
    def _build_loss_weights(self, latents, weight_mask) -> torch.Tensor | None:
        ...
        if has_spatial:
            ...
            spatial_w = 1.0 + self.spatial_weight_scale * binary_mask  # (B, 1, H, W)
        else:
        ...
        return spatial_w * ch_w  # broadcast → (B, C, H, W)

    def _training_step_ddpm(self, latents, noise, bsz, prompt_embeds, attention_mask, weight_mask=None) -> torch.Tensor:
        ...
        combined_weights = self._build_loss_weights(latents, weight_mask)
    ...
    def _run_validation(self, val_loop: ValidationLoop, val_gt_images=None):
        ...
        if val_loop.img2img_strengths and val_gt_images:
            # EulerDiscreteScheduler 支持显式传入 timesteps=[...]；
            # DPMSolver + use_karras_sigmas=True 不兼容该用法，因此 img2img 单独使用 Euler。
            from diffusers import EulerDiscreteScheduler
            img2img_scheduler = EulerDiscreteScheduler.from_config(
                self.noise_scheduler.config
            )
            img2img_data = []
        ...
        val_loop.run(
            ...
            img2img_scheduler=img2img_scheduler,
        )
        del pipeline
    ...
    def _maybe_save_best(self):
        ...
        if self._ema_loss < self._best_ema_loss:
            ...
            if self.accelerator.is_main_process:
                ...
                save_lora_weights(unwrapped, str(best_dir / "lora_transformer.safetensors"))
