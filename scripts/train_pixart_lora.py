#!/usr/bin/env python3
"""PixArt-Sigma LoRA 微调训练入口脚本。

用法:
  # 单卡训练
  python scripts/train_pixart_lora.py --config configs/lora_pixart_sigma_floorplan.yaml

  # 多卡训练 (accelerate)
  accelerate launch scripts/train_pixart_lora.py --config configs/lora_pixart_sigma_floorplan.yaml

  # 恢复训练
  accelerate launch scripts/train_pixart_lora.py \
    --config configs/lora_pixart_sigma_floorplan.yaml --resume latest
"""
...
from trainers.pixart_lora_trainer import PixArtLoRATrainer


def parse_args():
    parser = argparse.ArgumentParser(description="Train PixArt-Sigma LoRA")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
...
def main():
    ...
    trainer = PixArtLoRATrainer(config)
    trainer.train()
