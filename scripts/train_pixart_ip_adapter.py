#!/usr/bin/env python3
"""PixArt-Sigma IP-Adapter 训练入口脚本。

用法:
  # 多卡训练 (冻结 Transformer, 只训 IP-Adapter)
  accelerate launch scripts/train_pixart_ip_adapter.py --config configs/pixart_sigma_ip_adapter.yaml

  # 恢复训练
  accelerate launch scripts/train_pixart_ip_adapter.py \
    --config configs/pixart_sigma_ip_adapter.yaml --resume latest

  # 全量微调模式
  accelerate launch scripts/train_pixart_ip_adapter.py \
    --config configs/pixart_sigma_ip_adapter.yaml \
    --override training.freeze_transformer=false
"""
...
from trainers.pixart_ip_adapter_trainer import PixArtIPAdapterTrainer


def parse_args():
    parser = argparse.ArgumentParser(description="Train PixArt-Sigma with IP-Adapter")
...
def main():
    ...
    trainer = PixArtIPAdapterTrainer(config)
    trainer.train()
